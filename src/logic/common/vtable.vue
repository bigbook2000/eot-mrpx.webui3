...
<script setup lang="ts">
    ...
    const get_select_data = (confirm?: boolean): any => {
        ...
        //console.log("**** ****", "vtable::get_select_data", v_table.value?.getSelectionRows());
        if (confirm) {
    ...
    const get_select_index = (confirm?: boolean): number => {
        ...
        if (m_select_data == undefined) return -1;
        
        let dataId = m_select_data[props.idField];
        if (dataId == undefined) return -1;

        let list = x_list.value || [];
        let index = -1;
        for (let i = 0; i < list.length; i++) {
            if (list[i][props.idField] == dataId) {
                index = i;
                break;
            }
        }
    ...
    const load_list = (list: Array<any>): Array<any> => {
        ...
        if (list.length > 0) {
            let d = list[0];
            if (d["s_page_row_index"] < 0) {
                let n = eocore.to_int(d["s_total_count"]);
    ...
    const update_data = (
        ...
        dataCopy["_index"] = index;
        dataCopy["_insert"] = insert;

        if (props.onItem != undefined) {
            props.onItem(dataCopy);
        }

        if (insert) {

            // 如有指定索引，从索引处增加
            //console.log(index, x_list, data, dataCopy);

            if (index < 0)
                x_list.value.unshift(dataCopy);
            else
                x_list.value.splice(index, 0, dataCopy);
            ...
            if (index < 0) {
                ...
                if (index != -1) {
                    x_list.value[index] = Object.assign({}, x_list.value[index], dataCopy);
                    // 指向表格行
                    dataCopy = x_list.value[index];
                } else {
                    x_list.value.unshift(dataCopy);
                }
            }
    ...
    */
    ...
    const exchange_index = (index1: number, index2: number) => { 
        let data = x_list.value[index1];
        x_list.value[index1] = x_list.value[index2];
        x_list.value[index2] = data;
    }
    ...
    const onTableRowClick = (row: any, column: any, event: any) => {
        ...
        if (props.check == 'multi') {
            v_table.value?.toggleRowSelection(row);
        } else if (props.check == 'single') {
            v_table.value?.clearSelection();
            v_table.value?.toggleRowSelection(row, true);
        } 

        emits("row-click", m_select_data);
    }

    const onTableSortChange = (data: any) => {
