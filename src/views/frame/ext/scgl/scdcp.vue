<template>
    ...
    <div class="eo_col">
        ...
        <scdcp_xx ref="v_scdcp_xx" @close="onDialogClose_cgdcp_xx" />
    </div>
</template>
...
<script lang="ts" setup>
    ...
    type t_scdcp_xx = InstanceType<typeof scdcp_xx>;
    const v_scdcp_xx = ref<t_scdcp_xx>();
    ...
    const onTableItem_cgdcp = (data: any) => {
        ...
        // 价格格式化
        data["f_cpjg_s"] = eolib.fixed_num(data["f_cpjg"], 2);
        data["f_cgdj_s"] = eolib.fixed_num(data["f_cgdj"], 2);
        data["f_cgzj_s"] = eolib.fixed_num(data["f_cgdj"] * data["f_cgsl"], 2);

        const kcdj = eolib.divide_num(data["f_kczj"], data["f_cgsl"])
        data["f_kcdj_s"] = eolib.fixed_num(kcdj, 2);
    ...
    const onButtonClick_Add_cgdcp = async () => {
        ...
        v_scdcp_xx.value?.showDialog("产品信息", cgdcpData, m_field_array, true);
    }
    ...
    const onButtonClick_Upd_shrk = () => {
        ...
        v_scdcp_xx.value?.showDialog("产品入库", cgdcpData, [], false);
    }
    ...
    const onButtonClick_Get_cgdcp = () => {
        ...
        v_scdcp_xx.value?.showDialog("产品信息", cgdcpData, [], false);
    }
