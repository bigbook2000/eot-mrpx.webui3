        ...
        <div class="eo_col">
            ...
            <div class="eo_col_f">
                <vtable ref="v_table_user" 
                    ...
                    <el-table-column prop="f_role_s" label="角色" width="200"  show-overflow-tooltip />
                    <el-table-column prop="f_sex" label="性别" width="70" />
                    <el-table-column prop="f_phone" label="电话" width="140" />
                    <el-table-column prop="f_location" label="地址" width="280"  show-overflow-tooltip />
                    <el-table-column />
                </vtable>
            </div>
        </div>
...
<script lang="ts" setup>
    ...
    /** 查询条件 */
    let x_query_login_id = ref("");
    let x_query_name = ref("");
    let x_query_phone = ref("");
    ...
    const netLoad_user_query = () => {
        ...
        v_table_user.value!.load_list_net("/framework/user/query", {
    ...
    const onButtonClick_Del = async () => {
        v_table_user.value!.remove_data_net_select("/framework/user/del", (data) => {
            return {
    ...
    const onButtonClick_Reset = async () => {
        ...
        let ret: any = await eocore.post("/framework/user/reset", [{
    ...
    const onTableItem_user = (data: any) => {
        ...
        try {
            let dataEx = data["f_data_ex"];
            if (!eocore.check_empty(dataEx)) {
                let json = JSON.parse(dataEx);
    ...
    const onDialogClose_userinfo = async (cancel: boolean, data: any, cb: cfunc_boolean) => {
        ...
        v_table_user.value?.update_data_net("/framework/user/upd", {
            "f_user_id": userId,
            "f_login_id": loginId,
            "f_login_psw": "",
            "f_name": data["f_name"],
