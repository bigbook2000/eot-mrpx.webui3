...
<script lang="ts" setup>
    ...
    onMounted(async () => {
        ...
        v_table_role.value!.load_list_net("/framework/role/list", {});
        v_table_permit.value!.load_list_net("/framework/permit/list", {});
    });
    ...
    const onButtonClick_Del = () => {
        v_table_role.value!.remove_data_net_select("/framework/role/del", (data) => {
            return {
    ...
    const onButtonClick_Save = async () => {
        ...
        let ret: any = await eocore.post("/framework/role/permits", [{
    ...
    const onFormdClose_role = (cancel: boolean, data: any, cb: cfunc_boolean): void => {
        ...
        v_table_role.value!.update_data_net(
            "/framework/role/upd", data, -1, data["f_role_id"]<=0, true);
        cb(true);
    }
