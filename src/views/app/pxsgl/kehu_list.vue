<template>
    <!-- 客户选择对话框 - App端 -->
    <el-dialog v-model="x_visible" title="选择客户" fullscreen
            ...
            <div class="eo_col">
                ...
                <div class="div_search_bar">
                    <div class="search_input">
                        <el-input v-model="x_query_khmc" placeholder="搜索客户名称或联系方式"
                    ...
                    <div class="eo_scroll_v">
                        <div class="ap_list">
                            ...
                            <div v-for="item in x_data_list" :key="item.f_khgl_id"
                                ...
                                @click="x_selected_id = item.f_khgl_id">
                                <div class="check" @click.stop="onItemCheck(item)">
                                    <el-checkbox :model-value="x_selected_id === item.f_khgl_id" />
                                </div>
                                <div class="body">
                                    <div class="row">
...
<script lang="ts" setup>
    ...
    const onConfirm = () => {
        if (x_selected_id.value === 0) {
            eocore.show_info("请选择一个客户");
            return;
        }
        const selected = x_data_list.value.find(d => d.f_khgl_id === x_selected_id.value);
        if (!selected) {
            eocore.show_info("请选择一个客户");
            return;
        }
    ...
    const onItemCheck = (item: any) => {
        if (x_selected_id.value === item.f_khgl_id) {
            x_selected_id.value = 0;
        } else {
