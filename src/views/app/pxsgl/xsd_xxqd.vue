<template>
    ...
    <div class="eo_col" v-loading="x_show_loading">
        <!-- 工具栏按钮 -->
        <div class="eo_col_d eo_form" v-show="props.editMode == 1">
            <div class="button">
        ...
        <div class="eo_col_f">
            <div class="eo_scroll_v">
                <div class="ap_list">
                    ...
                    <div v-for="item in x_data_list" :key="item.f_xsdcp_id"
                        class="item"
                        :class="{ 'ap_sel': x_selected_id == item.f_xsdcp_id }"
                        @click="x_selected_id = item.f_xsdcp_id">
                        <div class="body">
                            <div class="row">
                            ...
                            <div class="row" v-if="props.editMode == 1">
...
<script lang="ts" setup>
    ...
    const onAdd_xsdcp = () => {
        ...
        v_xsdcp_xx.value?.showDialog(xsdcpData, props.fieldArray);
    }
    ...
    const onDel_xsdcp = async (xsdcpId?: number) => {
        ...
        const id = xsdcpId ?? x_selected_id.value;
        if (id <= 0) {
    ...
    const onUpd_xsdcp = (xsdcpId?: number) => {
        ...
        const item = x_data_list.value.find(d => d.f_xsdcp_id == id);
        if (!item) {
            eocore.show_info("未找到所选产品");
            return;
        }
        v_xsdcp_xx.value?.showDialog({ ...item }, props.fieldArray);
    }
