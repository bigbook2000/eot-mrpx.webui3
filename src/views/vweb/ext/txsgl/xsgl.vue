...
<script lang="ts" setup>
    ...
    import xsdcp from "@/views/vweb/ext/txsgl/xsdcp.vue"
    import xsdck from "@/views/vweb/ext/txsgl/xsdck.vue"
    ...
    const onButtonClick_Flow_Add = async (point: cflow_point, data0: any) => {
        ...
        let xsdData = TLogic.getEmptyData_xsd();
    ...
    const onButtonClick_Flow_XSTJ = async (point: cflow_point, data0: any) => {
        let dataNew = await TLogic.netLoad_xsd_count(data0["f_xsd_id"]);
        if (dataNew == undefined) return;
