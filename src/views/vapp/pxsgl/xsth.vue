<template>
    ...
    <div class="eo_page" v-loading="x_show_loading">
        <div class="eo_col">
            <topbar title="销售退货" :back="false" />
            <div class="eo_col_d">
                <!-- 简洁搜索栏 -->
                <div class="div_search_bar">
                    <el-button type="primary" @click="onButtonClick_Add_xsth">新增</el-button>
            ...
            <div class="eo_col_f">
                <div class="eo_scroll_v">
                    ...
                    <div class="ap_list">
                        ...
                        <div v-for="item in x_data_list" :key="item.f_xsth_id"
                            class="item"
                            :class="{ 'ap_sel': x_selected_id == item.f_xsth_id }"
                            @click="x_selected_id = item.f_xsth_id">
                            <div class="body">
                                <div class="row">
                                ...
                                <div class="row">
                                    ...
                                    <span class="value">{{ item.f_khmc || '-' }}</span>
                                </div>
                                <div class="row">
...
<script lang="ts">
/** KeepAlive */
export default { name: "app_xsgl_xsth" }
</script>
...
<script lang="ts" setup>
    ...
    onMounted(() => {
        m_flow_type = eoflow.get_type_by_name("销售退货");
        netLoad_xsth_query(-1);
    })

    onActivated(() => {
        netLoad_xsth_query(x_page_index.value - 1)
    })
    ...
    const netLoad_xsth_query = async (pageIndex: number = -1) => {
        let pageRowCount = x_page_row_count.value;
        let rowIndex = pageIndex * pageRowCount;
        if (pageIndex < 0) x_page_index.value = 1;

        x_show_loading.value = true;
        let ret = await eocore.proc("p_xsth_query", {
            "v_xsy_id": -1,
    ...
    const onItemDetailClick = (item: any) => {
        x_selected_id.value = item.f_xsth_id;
        router.push({ name: 'app_xsgl_xsth_xx', query: { id: item.f_xsth_id } })
    }

    const onButtonClick_Add_xsth = () => {
        router.push({ name: 'app_xsgl_xsth_xx', query: { id: 0 } });
    }
