...
export default {
	...
	get_sign(): string {
		...
		let sign = "";
		const list  = ['0', 'a', 'b', 'c', 'd', 'e', 'f', '1', '2', '3', '4', '5', '6', '7', '8', '9'];
		const len = this.token.length;
		for (let i = 0; i < len; i++) {
			const c = this.token[i];
			const n1 = list.indexOf(c);
			const n2 = Math.round(Math.random() * list.length);
			sign += list[n2 % list.length];
			sign += list[(n1 + n2) % list.length];
		}
    ...
    show_confirm(msg: string): Promise<boolean> {
        ...
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm(msg, "提示",
            {
				...
				// 如果拒绝，则抛出异常
                //reject(false);
				// let ret = await eocore.show_confirm("确定要取消吗？").catch(() => {
				//     // 用户点击了取消按钮
				//     cb(true); return;
				// });
            })		
